...
func handle_jump():
	...
	if is_on_floor() or coyote_jump_timer.time_left > 0.0:
		...
		if Input.is_action_just_pressed("up") and air_jump and not just_wall_jumped:
...
func handle_acceleration(input_axis, delta):
	...
	if input_axis:
		if Input.is_action_pressed("run"):
			velocity.x = move_toward(velocity.x, 3 * movement_data.speed * input_axis, movement_data.acceleration * delta)
		else:
